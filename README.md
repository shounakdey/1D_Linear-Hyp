# 1D_Linear-Hyp
Solver for Simple Linear Hyperbolic Equation with constant velocity $\left[\frac{\partial u}{\partial t} + c\frac{\partial u}{\partial x} = 0 \right]$. The program contains the following algorithms - 
* Upwind Scheme
* Lax-Wendroff Scheme
* MacCormack Scheme
...
The code has three sections, with the variables (Datastruct.cpp) and functions (Functions.cpp) being defined in separate files, while the main function (Combined_Schemes.cpp) containing only the functions. The parameters are extracted from ```Read_File.txt```, which can be changed as per requirement.
